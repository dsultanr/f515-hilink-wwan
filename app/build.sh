#!/bin/bash
# Builds ModemGuide.apk without gradle: aapt2 -> javac -> d8 -> zipalign -> apksigner.
set -euo pipefail
...
if [ ! -f "$PROJ/keystore.jks" ]; then
    keytool -genkeypair -keystore "$PROJ/keystore.jks" -alias modemguide \
        -storepass modemguide -keypass modemguide -keyalg RSA -keysize 2048 \
        -validity 10000 -dname "CN=ModemGuide, O=f515, C=RU" >/dev/null 2>&1
    echo "   (created keystore.jks)"
fi
"$BT/zipalign" -f -p 4 "$OUT/unsigned.apk" "$OUT/aligned.apk"
"$BT/apksigner" sign --ks "$PROJ/keystore.jks" --ks-pass pass:modemguide \
    --key-pass pass:modemguide --v1-signing-enabled true --v2-signing-enabled true \
